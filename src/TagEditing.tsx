...
import type { TagEditingProps, TagEditingRef } from './typings';

import { sleep } from './utils';

function TagEditing(props: TagEditingProps, ref: React.ForwardedRef<TagEditingRef | null>) {
  const { placeholder, value, onChange, onPrev, onNext, onDelete, index, locationTo } = props;
  ...
  const handleKeyDown = useCallback(
    async (e: React.KeyboardEvent) => {
      ...
      if (e.key === 'Backspace') {
        ...
        if (range.startOffset === range.endOffset && range.endOffset === input.current?.textContent?.length) {
          e.preventDefault();
          e.stopPropagation();
          if (!(await handleBlur())) {
            onNext();
...
export default React.forwardRef(TagEditing);
